    ...
    for (j in i:ts_length)
    {
      ...
      } else if (j==i+1)
      {
        ...
        for (cluster_id in 1:Nattr)
        {
          ...
          # For the segment length cluster
          length_of_current_segment <- j - i + 1
          alphak <- clusters$clusters_l[[cluster_id]]$alpha 
          betak <- clusters$clusters_l[[cluster_id]]$beta
          ...
          p_l_in_cluster[cluster_id] <- beta(length_of_current_segment + alphak, betak + 1)
...
get_cluster_posterior_geometric_params_of_cluster_k <- function(lk, alpha0, beta0)
{
  nk <- length(lk)
  alphak <- alpha0 + sum(lk)
  betak <- beta0 + nk
  return(list("alpha" = alphak, "beta" = betak))
}

# We use a geometric distribution on the length of the segment
get_clusters_to_use_in_dynamic_programming <- function(Nattr, attr, a_cut, l_cut, hypa, index_of_ts_to_exclude)
{
