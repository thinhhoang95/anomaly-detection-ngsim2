...
debugSource("~/Documents/anomaly-detection-ngsim/lanechange/vwp_libc_wishart_geo.R")
...
hypa <- list("mu" = mu0, "nu" = 1, "kappa" = 5, "lambda" = 0.5, "alpha0" = 1, "alpha0" = 10, "beta0" = 3)
  ...
  for (ts_index in 1:(dim(x_mat)[1]))
  {
    ...
    for (cp_table in 1:(Nattr_cp+1))
    {
      if (cp_table <= Nattr_cp)
      {
        ...
        clusters <- get_clusters_to_use_in_dynamic_programming(Nattr, attr, a_cut, l_cut, hypa, ts_index)
...
for (i in 1:nrow(x_mat))
{
  if (i==1)
  {
    plot(1:ncol(x_mat), x_mat[i,], type='l', ylim=c(min(x_mat), max(x_mat)), xlab='Time sample (0.1s)', ylab='Lateral coordinate (ft)')
  }
