...
# Author: Thinh Hoang (dinhthinh.hoang@nxp.com)
# ================
#
...
zero_period <- 12
rise_period <- 5
for (s in 1:10) # total number of time series to generate
{
...
for (s in 11:30) # total number of time series to generate
{
  slope_start_from <- sample(1:4, size=1, replace=T)
  x_sample <- rep(0, default_ts_length)
  for (t in 1:length(x_sample))
...
alpha_cp <- 1 # the rate at which the time series will decide to open a larger number of changepoints that other time series has not yet considered
