...
cut_x <- function(x_mat, cp, basis, pa_homo)
{
  ...
  for (ts_index in 1:dim(x_mat)[1])
  {
    ...
    for (i in 1:(length(cp_of_this_ts)+1))
    {
      segment_starts_at <- cp_of_this_ts_extended[i]
      segment_ends_at <- cp_of_this_ts_extended[i+1]
      length_of_this_segment <- segment_ends_at - segment_starts_at + 1
      segment <- x_mat[ts_index, segment_starts_at:segment_ends_at]
      segments_of_this_ts <- append(segments_of_this_ts, list(segment))
      a <- pa_homo$a[ts_index, segment_starts_at, segment_ends_at]
      as_of_this_ts <- append(as_of_this_ts, list(a))
      ls_of_this_ts <- append(ls_of_this_ts, list(length_of_this_segment))
    }
    segments <- append(segments, list(segments_of_this_ts))
    as <- append(as, list(as_of_this_ts))
    ls <- append(ls, list(ls_of_this_ts))
  }
  return(list("x_cut" = segments, "a_cut" = as, "l_cut" = ls))
}
...
# >>> This modified version uses the multivariate t-distribution!
precalculate_a_and_likelihood <- function(ts_index, ts_length, pa_homo, basis, vary, Nattr, clusters, segment_seatings, alpha)
{
  # so the time series ts would start from zero, not from any arbitrary value
  a_array <- array(NA, dim=c(ts_length, ts_length))
  p_array <- array(NA, dim=c(ts_length, ts_length))
  cmu_l <- rep(0, Nattr)
  for (i in 1:(ts_length-1))
  {
    for (j in i:ts_length)
    {
      ...
      } else if (j==i+1)
      {
        ...
        p_a_in_cluster <- rep(0, Nattr)
        p_l_in_cluster <- rep(0, Nattr)
        p_typicality_in_cluster <- rep(0, Nattr)
        for (cluster_id in 1:Nattr)
        {
          # p_a_in_cluster[cluster_id] <- -1/2 * ctau * (a - cmu[cluster_id])^2
          # Calculate the params for the t distribution from the clusters
          
          mun <- clusters$clusters_t[[cluster_id]]$mu 
          sigman <- (clusters$clusters_t[[cluster_id]]$lambda * (clusters$clusters_t[[cluster_id]]$kappa + 1))/(clusters$clusters_t[[cluster_id]]$kappa * (clusters$clusters_t[[cluster_id]]$nu - 1 + 1))
          dfn <- clusters$clusters_t[[cluster_id]]$nu - 1 + 1
          # For the segment length cluster: parameters for the posterior predictive
          length_of_current_segment <- j - i + 1
          mun_l <- clusters$clusters_l[[cluster_id]]$mu 
          sigman_l <- (clusters$clusters_l[[cluster_id]]$lambda * (clusters$clusters_l[[cluster_id]]$kappa + 1))/(clusters$clusters_l[[cluster_id]]$kappa * (clusters$clusters_l[[cluster_id]]$nu - 1 + 1))
          dfn_l <- clusters$clusters_l[[cluster_id]]$nu - 1 + 1
          cat('Cluster: ', cluster_id, '\n')
          cat('Variance of cluster segment length: ', sigman_l, '\n')
          # Update the cluster mean length to show it
          cmu_l[cluster_id] <- mun_l
          
          # The typicality of this segment length follows a beta distribution
          # in particular, the predictive posterior probability of having length_of_current_segment
          # conditioned on all the segment length assigned to the current cluster
          p_l_in_cluster[cluster_id] <- dmvt(length_of_current_segment, mun_l, as.matrix(sigman_l), dfn_l, log=F)
          ...
          p_typicality_in_cluster[cluster_id] <- p_popularity_of_the_current_cluster * p_a_in_cluster[cluster_id] * p_l_in_cluster[cluster_id]
  ...
  return(list("a" = a_array, "p" = p_array, "cmu_l" = cmu_l))
}
...
dynamic_program_cp <- function(sol, Ncp, ts, ts_index, pa_homo, k, basis, vary, Nattr, clusters, segment_seatings, alpha)
{
  ...
  ts_pah <- precalculate_a_and_likelihood(ts_index, ts_length, pa_homo, basis, vary, Nattr, clusters, segment_seatings, alpha)
  ...
  if (k < Ncp)
  {
    ...
    sol <- dynamic_program_cp(sol, Ncp, ts, ts_index, pa_homo, k+1, basis, vary, Nattr, clusters, segment_seatings, alpha)
  }
  return(sol)
}
...
get_cluster_posterior_student_params_of_cluster_k <- function(ak, mu0, nu0, kappa0, lambda0)
{
  # This function returns all the parameters (mu_n, kappa_n, nu_n, Lambda_n) corresponding to the cluster k
  # ak: all the representations assigned to cluster k, mu0: prior mean, nu0: degrees of freedom, kappa0: number of prior measurements, lambda0: scale matrix
  # To call this function, it requires to get all the representations associated with cluster k
  n <- length(ak)
  ybar <- mean(ak)
  S <- sum((ak-ybar)^2) # sum of square of y-bar{y}
  mu_n <- kappa0/(kappa0+n) * mu0 + n/(kappa0 + n) * ybar
  kappa_n <- kappa0 + n
  nu_n <- nu0 + n
  lambda_n <- lambda0 + S + kappa0*n/(kappa0 + n) * (ybar - mu0)^2
  return(list("mu" = mu_n, "kappa" = kappa_n, "nu" = nu_n, "lambda" = lambda_n))
}

# We use a geometric distribution on the length of the segment
get_clusters_to_use_in_dynamic_programming <- function(Nattr, attr, a_cut, l_cut, hypa, hypa_l, index_of_ts_to_exclude)
{
  ...
  l_ex <- unlist(l_cut[-index_of_ts_to_exclude]) # the length of 
  ...
  for (k in 1:Nattr)
  {
    ...
    l_of_k <- l_ex[as.vector(attr_ex == k)] # get all segment length associated with cluster k
    ...
    # For the beta-geometric distribution
    l_param <- get_cluster_posterior_student_params_of_cluster_k(ak = l_of_k, mu0 = hypa_l$mu, nu0 = hypa_l$nu, kappa0 = hypa_l$kappa, lambda0 = hypa_l$lambda)
    # l_param <- get_cluster_posterior_geometric_params_of_cluster_k(lk = l_of_k, alpha0 = hypa$alpha0, beta0 = hypa$beta0)
    clusters_t <- append(clusters_t, list(t_param))
    clusters_l <- append(clusters_l, list(l_param))
  }
  # The output of the function include two parameter set, one for the student-t distribution,
  # the other for the geometric distribution for the typical length of the cluster segments
  return(list("clusters_t" = clusters_t, "clusters_l" = clusters_l))
}